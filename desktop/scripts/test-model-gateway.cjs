...
const {ModelGatewayBridge, inferCapabilities, endpointUrl, generationRoute, PROTOCOLS} = require("../src/main/model-gateway-bridge.cjs");
...
(async()=>{
  ...
  await test("模型级完整查询地址不会与调用地址重复拼接",()=>assert.equal(endpointUrl("https://api.example.com/v1/videos","https://query.example.com/tasks/job-1"),"https://query.example.com/tasks/job-1"));
  await test("模型网关不再推断生成地址，完全使用管理员配置",()=>{const route=generationRoute({baseUrl:"https://gateway.example.com/v1/videos",videosPath:"",videoStatusPath:"",videoCancelPath:"",protocol:"custom-json"},"video");assert.equal(route.submitPath,"");assert.equal(route.statusPath,"");assert.equal(route.cancelPath,"");});
  ...
  await test("连接测试失败状态持久化",async()=>{const failed=bridgeA.createProvider({name:"失败厂商",protocol:"custom-json",baseUrl:"https://bad.example.com",modelsPath:"/v1/models"});const result=await bridgeA.testProvider(failed.id);assert.equal(result.ok,false);assert.equal(result.provider.status,"error");assert.equal(result.statusText,"mock unauthorized");});
